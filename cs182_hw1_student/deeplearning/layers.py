...
def affine_forward(x, w, b):
    ...
    #############################################################################
    x1 = np.copy(x)
    x1 = np.reshape(x1, (len(x1), x[0].size))
    b1 = np.vstack([b] * len(x))
    out = np.add(np.matmul(x1, w), b1)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, w, b)
...
def relu_forward(x):
    ...
    out = np.vectorize(relu, otypes=[float])(x)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = x
...
def relu_backward(dout, cache):
    ...
    #############################################################################
    def relu_indicator(y):
        if y < 0: return 0
        elif y >= 0: return 1
    x = np.vectorize(relu_indicator)(x)    
    dx = x * dout
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for f in range(F):
            for i in range(0,H - HH + 1, conv_param['stride']):
                for j in range(0, W - WW + 1, conv_param['stride']):
                  s = np.sum(x[n, :, i:i+HH, j:j+WW] * w[f, :, :, :])
                  s += b[f]
          ...
          for l in range(WW):
            ...
            for i in range(0, H - HH + 1, stride):
              for j in range(0, W - WW + 1, stride):
                for n in range(N):
                    # print(dout.shape, W - WW + 1)
                    s += x[n, c, i:i+HH, j:j+WW][k, l] * dout[n, f, int(i / stride), int(j / stride)]
            dw[f, c, k, l]  = s
        ...
        for f in range(F):
          for i in range(0, H - HH + 1, conv_param['stride']):
            for j in range(0, W - WW + 1, conv_param['stride']):
                for k in range(HH):
                   for l in range(WW):
                      dx[n, c, i + k, j + l] += dout[n, f, int(i / stride), int(j / stride)] * w[f, c, k, l]
...
def max_pool_forward_naive(x, pool_param):
    ...
    out = np.zeros((N, C, int((H - F) / stride) + 1, int((W - F) / stride) + 1))
    for n in range(N):
       for c in range(C):
          for i in range(0, H - F + 1, stride):
             for j in range(0, W - F + 1, stride):
                out[n, c, int(i / stride), int(j / stride)] = np.max(x[n, c, i:i + F, j:j+F])
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
       for c in range(C):
          for i in range(0, H - F + 1, stride):
             for j in range(0, W - F + 1, stride):
                m = np.max(x[n, c, i:i + F, j:j+F])
                for k in range(F):
                   for l in range(F):
                      if x[n, c, i + k, j + l] == m:
                        dx[n, c, i + k, j+ l] += dout[n, c, int(i / stride), int(j / stride)]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    #############################################################################
    x = cache['x']
    dx, dgamma, dbeta = np.zeros(dout.shape), np.zeros((x.shape[1], )), np.zeros((x.shape[1], ))
    # t = np.zeros((x.shape))
    # for c in range(C):
    #   t[:, c] = dout[:, c, :, :].flatten()
    t = dout.reshape(x.shape)
    dx, dgamma, dbeta = batchnorm_backward(t, cache)
